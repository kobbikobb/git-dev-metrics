...
Expert review of architecture, testing, CI/CD, packaging, and best practices.
...
def test_should_return_one_day_ago_for_1d_period(self):
    ...
    expected = datetime(2024, 6, 14, 12, 0, 0)
    assert is_same_date(result, expected)
```

---

## 2. Clean Up pyproject.toml Dependencies

**File:** `pyproject.toml`

Three problems:

### a) Dev tools in runtime dependencies (lines 10, 14)
`pyright` and `ruff` are linting/type-checking tools. Anyone who `pip install`s your package gets them too. Remove them from `[project.dependencies]`.

### b) Unused dependency (line 8)
`fastapi` is never imported anywhere. Remove it unless you have future plans for it.

### c) Duplicate deps with version conflicts
`pytest-cov`, `pytest-mock`, `responses` appear in both `[project.optional-dependencies] test` and `[dependency-groups] dev` with different version numbers.

**Fix:** Delete `[project.optional-dependencies]` entirely. Keep everything in `[dependency-groups]`:
...
Run `uv sync` after making changes.
